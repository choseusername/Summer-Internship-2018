...
function updateEntities(dt) {
    ...
    for (var i = 0; i < enemies.length; i++) {
        if (enemies[i].dir === 'forward') {
            enemies[i].pos[0] -= enemySpeed * dt;
        } else if (enemies[i].dir === 'up') {
...
// Remove colliding bullets with megaliths
function removeCollidingBulletsWithMegalith(megalith) {
    bullets.some(function(bullet, index, bullets) {
        if (boxCollides(megalith.pos, megalith.sprite.size, bullet.pos, bullet.sprite.size)) {
...
function checkMegalithsCollisions() {
    megaliths.forEach(function(megalith) {
        removeCollidingBulletsWithMegalith(megalith);

        // Game over if collide with megalith
        if (boxCollides(megalith.pos, megalith.sprite.size, player.pos, player.sprite.size)) {
            gameOver();
        }
    });
}

// Enemys circumambulate megaliths
function circumambulateMegaliths() {
    enemies.forEach(function(enemy) {
        var isCollided = megaliths.some(function(megalith, index, megaliths) {
            return boxCollides(enemy.pos, enemy.sprite.size, megalith.pos, megalith.sprite.size);
        });
...
// Create megalith
var MegalithType = Object.freeze({
    "type1": 1,
    "type2": 2
})
